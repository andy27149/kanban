...
> **2026-08-24 更正说明**：本文档定的是架构（Flask + config.yaml 驱动 + data.json 快照），架构本身仍然有效。但下方"配置映射文件"示例中的文件名/字段（`经营数据.xlsx`/`库存台账.xlsx`/`销售明细.xlsx`/"总营收"/"总利润"等）是早期编造的示例，不对应任何真实数据——实际唯一的真实数据源是 `基础表.xlsx`。**当前实际展示内容以 [`2026-08-24-boss-dashboard-content-plan.md`](2026-08-24-boss-dashboard-content-plan.md) 为准**，该文档已经用户确认并落地实现。另外，`fixed_range` 取数模式已从"仅 KPI"扩展为 KPI/图表/表格通用，用于处理 `合计`/`出库明细` 等多行合并表头的 sheet。
